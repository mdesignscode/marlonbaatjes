<script lang="ts">
	...
	function modulo(n: number, m: number) {
        ...
        const setPrevIndex = () => currentIndex.update(() => $currentIndex === 1 ? images.length - 1 : modulo($currentIndex - 1, imagesLen));
        const setNextIndex = () => currentIndex.update(() => $currentIndex === images.length ? 1 : $currentIndex + 1);
</script>

<button
        aria-label="Show previous design"
        class={[
                "transition-all cover__image image-left",
                navigating
                    ? "translate-x-4"
                    : "group-hover:-translate-x-28 group-hover:-rotate-[10deg]"
        ]}
        onclick={() => {
          navigating = true;
          setTimeout(() => {
...
<button
        ...
        class={[
                "transition-all cover__image image-right",
                navigating
                    ? "-translate-x-4"
                    : "group-hover:-translate-x-28 group-hover:-rotate-[10deg]"
        ]}
...
<style>
        .cover__image {
                @apply object-contain w-[25%] relative rounded-md;
        }

        .image-left, .image-right {
                @apply scale-[0.9] z-30;
        }
